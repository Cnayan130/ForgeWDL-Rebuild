...
import top.principlecreativity.fgwdlrb.api.IWDLMessageType;
...
public class WDLMessages {
	...
	public static void registerMessage(String name, IWDLMessageType type,
		...
		WorldDownloader.defaultProps.setProperty("Messages." + name,
				Boolean.toString(type.isEnabledByDefault()));
		WorldDownloader.defaultProps.setProperty("MessageGroup." + category.internalName,
				"true");
	}
	...
	public static boolean isEnabled(IWDLMessageType type) {
		...
		if (!WorldDownloader.baseProps.containsKey("Messages." + r.name)) {
			if (WorldDownloader.baseProps.containsKey("Debug." + r.name)) {
				//Updating from older version
				WorldDownloader.baseProps.put("Messages." + r.name,
						WorldDownloader.baseProps.remove("Debug." + r.name));
			} else {
				WorldDownloader.baseProps.setProperty("Messages." + r.name,
						Boolean.toString(r.type.isEnabledByDefault()));
			}
		}
		return WorldDownloader.baseProps.getProperty("Messages." + r.name).equals("true");
	}
	...
	public static boolean isGroupEnabled(MessageTypeCategory group) {
		...
		return WorldDownloader.baseProps.getProperty(
	...
	public static void toggleGroupEnabled(MessageTypeCategory group) {
		WorldDownloader.baseProps.setProperty("MessageGroup." + group.internalName,
	...
	public static void resetEnabledToDefaults() {
		WorldDownloader.baseProps.setProperty("Messages.enableAll", "true");
		enableAllMessages = WorldDownloader.globalProps.getProperty("Messages.enableAll",
				"true").equals("true");

		for (MessageRegistration r : registrations) {
			WorldDownloader.baseProps.setProperty(
					"MessageGroup." + r.category.internalName,
					WorldDownloader.globalProps.getProperty("MessageGroup."
							+ r.category.internalName, "true"));
			WorldDownloader.baseProps.setProperty(
					"Messages." + r.name,
					WorldDownloader.globalProps.getProperty("Messages." + r.name));
		}
	}
	...
	public static void onNewServer() {
		if (!WorldDownloader.baseProps.containsKey("Messages.enableAll")) {
			if (WorldDownloader.baseProps.containsKey("Debug.globalDebugEnabled")) {
				//Port from old version.
				WorldDownloader.baseProps.put("Messages.enableAll",
						WorldDownloader.baseProps.remove("Debug.globalDebugEnabled"));
			} else {
				WorldDownloader.baseProps.setProperty("Messages.enableAll",
						WorldDownloader.globalProps.getProperty("Messages.enableAll", "true"));
			}
		}

		enableAllMessages = WorldDownloader.baseProps.getProperty("Messages.enableAll")
				.equals("true");
	}
